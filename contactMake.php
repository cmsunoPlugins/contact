...
if(file_exists('data/_sdata-'.$sdata.'/'.$busy.'/contact.json')) {
	// {"t":"te","l":"Pr\u00e9nom"}
	$q1 = file_get_contents('data/_sdata-'.$sdata.'/'.$busy.'/contact.json');
	$a1 = json_decode($q1,true);
	$o1 = "\r\n".'<div class="'.(isset($Uw3['input']['w3-container'])?$Uw3['input']['w3-container']:'w3-container').(!empty($a1['clas'])?' '.$a1['clas']:'').' contactBloc" id="contactBloc">'."\r\n\t".'<form>'."\r\n";
	$s1 = '';
	$s2 = '';
	if(!empty($a1['frm'])) foreach($a1['frm'] as $k1=>$v1) {
	...
	$o1 .= "\t\t".'<div><button class="'.(isset($Uw3['input']['w3-button'])?$Uw3['input']['w3-button']:'w3-button').'" type="button" onClick="contactSend(this);">'.$a1['send'].'</button></div>'."\r\n";
	$o1 .= "\t".'</form>'."\r\n".'</div><!-- #contactBloc -->'."\r\n";
